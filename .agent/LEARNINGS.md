...
- **长对话卡顿根因与修复**（1645 条实测）：原 release 切换耗时 5-8 秒，瓶颈在一次性构建全量 widget 树。已通过分页（首次 50 条）+ `reverse: true` ListView + 后台逐批加载解决，切换降至 <100ms
- **分页架构**：`ChatController._allMessageIds` 快照 + `_loadRecentMessages()`（最后 50 条 ID）+ `loadAllMessages()` 守卫。快照是一次性拷贝，同一对话内增删消息不会同步，靠 `hasMoreMessages` 守卫防止越界，切换对话时自愈
- **reverse ListView**：`reverse: true` 让 offset 0 = 物理底部（最新消息），后台追加历史消息到物理顶部不影响视口。滚动逻辑全部翻转：scrollToBottom → `jumpTo(0)`，scrollToTop → `jumpTo(max)`，`Scrollable.ensureVisible` alignment 从 0.08 翻为 0.92（0.0=底部，1.0=顶部）。paging step 方向也需翻转
- **`initChat()` 直接路径**：首次启动通过 `_chatController.setCurrentConversation()` 直接设置对话，不经过 `ViewModel.switchConversation()`，因此 `onConversationSwitched` 回调不触发。需要在 `initChat()` 中单独调 `_scheduleBackgroundMessageLoad()`
- **上游分页方案** (`48a1e41`, `upstream/pagination`)：初始加载 12 条效果显著，但 `buildApiMessages`/`forkConversation`/`compressContext` 等依赖全量 messages 未处理，直接用会导致 AI 丢上下文
...
- **延迟 UI 状态恢复**（`_restoreMessageUiState` 分批 + postFrameCallback）：仅省 ~160ms，实际卡顿来自渲染管线，4% 改善无感知，已回退
- **后台渐进加载（非 reverse 版本）**：逐批加载 + `jumpTo` 补偿 → 32 帧连续闪烁。改用 `reverse: true` 后此方案可行，已采用
