...
- 本地开发环境路径：Flutter SDK `D:\develop\flutter`、Android SDK `D:\develop\Android\Sdk`、JDK 17 `C:\Program Files\Eclipse Adoptium\jdk-17.0.18.8-hotspot`
...
- Gradle 8.12 不支持 JDK 25（版本号检测失败），需要 JDK 17 LTS。通过 `JAVA_HOME` 环境变量指定
- `sdkmanager` 在 JDK 25 下需要设置 `$env:SKIP_JDK_VERSION_CHECK = "true"` 才能运行
